...
import { addDishThunk, fetchDishesThunk } from "@/app/redux/features/dishes/dishSlice";
...
export default function AddProductModal({ onClose }: AddProductModalProps) {
  ...
  const onSubmit = async (formData: ProductFormData) => {
    ...
    try {
      await dispatch(addDishThunk(formDataToSend as any)).unwrap();
      dispatch(
        fetchDishesThunk({
          page: 1,
          limit: 10,
      ...
      setSnackbarMessage(error.message || "Error in adding Dish");
      setSnackbarOpen(true);
    }
    reset();
  };
